...
{
    [WebsiteAuthorize]
    public class CALCertificadoGranelTratamientoTermicoController : BaseApplicationController
    {
        private SoftwareCalidadDBDataContext dcSoftwareCalidad = new SoftwareCalidadDBDataContext();

        public CALCertificadoGranelTratamientoTermicoController()
        {
            SetCurrentModulo(10);
        }

        // GET: CALCertificadoGranelTratamientoTermico
        public ActionResult Index()
        {
            CheckPermisoAndRedirect(377);
            List<CAL_CCGTratamientoTermico> list = dcSoftwareCalidad.CAL_CCGTratamientoTermico.Where(X => X.Habilitado == true).OrderByDescending(X => X.FechaHoraIns).OrderByDescending(X => X.IdCCGTratamientoTermico).ToList();
        ...
        public ActionResult DescargarPDF(int id, string certkey)
        {
            ...
            try
            {
                ...
                return File(pdf, "application/pdf", String.Format("Certificado de Tratamiento Térmico {0}-{0}.pdf", tratamientoTermico.NCertificado, tratamientoTermico.FechaHoraIns.Year));
            }
            catch
            {
