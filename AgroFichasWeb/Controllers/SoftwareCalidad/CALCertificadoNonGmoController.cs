...
{
    [WebsiteAuthorize]
    public class CALCertificadoNonGmoController : BaseApplicationController
    {
        private SoftwareCalidadDBDataContext dcSoftwareCalidad = new SoftwareCalidadDBDataContext();
        private AgroFichasDBDataContext dcAgroFichas = new AgroFichasDBDataContext();

        public CALCertificadoNonGmoController()
        {
            SetCurrentModulo(10);
        }
        ...
        public ActionResult Index(int? id, int? IdPlantaProduccion)
        {
            ...
            List<CAL_CertificadoNonGmo> list = dcSoftwareCalidad.CAL_CertificadoNonGmo
                .Where(X => X.Habilitado == true && X.CAL_OrdenProduccion.IdPlanta == IdPlantaProduccionSelect)
                .OrderByDescending(X => X.FechaHoraIns)
                .OrderByDescending(X => X.IdCertificadoNonGmo).ToList();
        ...
        public ActionResult Crear()
        {
            ...
            CAL_CertificadoNonGmo cert = new CAL_CertificadoNonGmo();
            return View(cert);
        }

        [HttpPost]
        public ActionResult Crear(CAL_CertificadoNonGmo cert, FormCollection formCollection)
        {
            CheckPermisoAndRedirect(378);
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    dcSoftwareCalidad.CAL_CertificadoNonGmo.InsertOnSubmit(cert);
        ...
        {
            CheckPermisoAndRedirect(377);
            CAL_CertificadoNonGmo cert = dcSoftwareCalidad.CAL_CertificadoNonGmo.SingleOrDefault(X => X.IdCertificadoNonGmo == id && X.Habilitado == true);
            if (cert == null)
            {
                return RedirectToAction("Index", new { errMsg = "No se ha encontrado el certificado", okMsg = "" });
            }

            string htmlContent = System.IO.File.ReadAllText(Server.MapPath("~/App_Data/certificados/certificado-non-gmo.html"), Encoding.UTF8);
            RepTemp(ref htmlContent, "CONSIG", cert.CAL_OrdenProduccion.GetCliente(cert.CAL_OrdenProduccion.IdCliente).RazonSocial);
            ...
            RepTemp(ref htmlContent, "LOTE" , cert.CAL_OrdenProduccion.LoteComercial);
            ...
            try
            {
                ...
                {
                    IdTipoCertificado = 4,
                    IdCertificado     = cert.IdCertificadoNonGmo,
                ...
                return RedirectToAction("DescargarPDF", new { id = cert.IdCertificadoNonGmo, certKey = certKey });
            }
            catch (Exception ex)
            {
                // This code segment write data to file.
                string certKey = Guid.NewGuid().ToString();
                FileStream fileStream = new FileStream(Path.Combine(Server.MapPath("~/App_Data/pdfs/softwarecalidad/certificados"), string.Format("IDCERT_CERTIFICADONONGMO_{0}_{1}.txt", cert.IdCertificadoNonGmo, certKey)), FileMode.OpenOrCreate, FileAccess.Write);
                StreamWriter writer = new StreamWriter(fileStream);
        ...
        public ActionResult DescargarPDF(int id, string certkey)
        {
            ...
            try
            {
                ...
                return File(pdf, "application/pdf", String.Format("Certificado NON GMO {0}-{1}.pdf", cert.CAL_OrdenProduccion.LoteComercial, cert.FechaHoraIns.Year));
            }
            catch
            {
        ...
        public ActionResult Eliminar(int id)
        {
            ...
            try
            {
                ...
                dcSoftwareCalidad.SubmitChanges();
                okMsg = String.Format("El certificado {0} ha sido eliminado", cert.IdCertificadoNonGmo);
            }
            catch (Exception ex)
            {
                errMsg = ex.Message;
        ...
        public ActionResult ControlVersiones(int id)
        {
            ...
            List<CAL_CertificadoControlVersion> list = dcSoftwareCalidad.CAL_CertificadoControlVersion.Where(X => X.IdCertificado == cert.IdCertificadoNonGmo && X.IdTipoCertificado == 4 && X.Habilitado == true).OrderByDescending(X => X.FechaHoraIns).ToList();
            ViewData["errMsg"]          = Request["errMsg"];
            ViewData["okMsg"]           = Request["okMsg"];
            ViewData["permisosUsuario"] = new Permiso(CheckPermiso(378),
                                                      CheckPermiso(377),
        ...
        public ActionResult DescargarPDF_ControlVersion(int id, string certkey)
        {
            ...
            try
            {
                ...
                return File(pdf, "application/pdf", String.Format("Certificado NON GMO {0}-{1} V{2:dd-MM-yy HH;mm;ss}.pdf", cert.NCertificado, cert.FechaHoraIns.Year, controlVersion.FechaHoraIns));
            }
            catch
            {
