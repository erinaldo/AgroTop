...
{
    [WebsiteAuthorize]
    public class CALCertificadoGranelControlGroatsPorLoteController : BaseApplicationController
    {
        private SoftwareCalidadDBDataContext dcSoftwareCalidad = new SoftwareCalidadDBDataContext();

        public CALCertificadoGranelControlGroatsPorLoteController()
        {
            SetCurrentModulo(10);
        }

        // GET: CALCertificadoGranelControlGroatsPorLote
        public ActionResult Index()
        {
            CheckPermisoAndRedirect(377);
            List<CAL_CCGControlPorLote> list = dcSoftwareCalidad.CAL_CCGControlPorLote.Where(X => X.Habilitado == true).OrderByDescending(X => X.FechaHoraIns).OrderByDescending(X => X.IdCCGControlPorLote).ToList();
        ...
        public ActionResult DescargarPDF(int id, string certkey)
        {
            ...
            try
            {
                ...
                return File(pdf, "application/pdf", String.Format("Control de Groats Por Lote de Certificado de Calidad {0}-{0}.pdf", controlPorLote.NCertificado, controlPorLote.FechaHoraIns.Year));
            }
            catch
            {
