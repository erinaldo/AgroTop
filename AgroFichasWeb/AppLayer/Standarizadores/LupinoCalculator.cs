...
namespace AgroFichasWeb.AppLayer.Standarizadores
{
    public class LupinoCalculator
    {
        public static CalculatorResult Normalizar(AgroFichasDBDataContext dc, ProcesoIngreso ingreso)
        {
            ...
            var humedad = ingreso.ValorAnalisis.Single(va => va.IdParametroAnalisis == 37);
            var impureza = ingreso.ValorAnalisis.Single(va => va.IdParametroAnalisis == 38);
            ...
            var rangoHumedad = dc.RangoAnalisis.FirstOrDefault(ra => ra.IdParametroAnalisis == 37 && humedad.Valor.Value >= ra.Desde && humedad.Valor.Value <= ra.Hasta);
            var factorHumedad = (rangoHumedad != null ? rangoHumedad.Bonificacion : -humedad.Valor.Value);

            var rangoImpureza = dc.RangoAnalisis.FirstOrDefault(ra => ra.IdParametroAnalisis == 38 && impureza.Valor.Value >= ra.Desde && impureza.Valor.Value <= ra.Hasta);
            ...
            if (humedad.Valor.Value > 12)
                descuentoHumedadKg = (result.PesoBruto * ((humedad.Valor.Value - 12) / 100));
            if (impureza.Valor.Value > 2)
                descuentoImpurezaKg = (result.PesoBruto * ((impureza.Valor.Value - 2) / 100));
